...
static const char* connectionString = "[iothub connectionString]";
...
void iothub_client_sample_mqtt_run(int judge)
{
    ...
    else
    {
        ...
        else
        {
			...
			if (judge) {
				if (IoTHubClient_LL_SetOption(iotHubClientHandle, "TrustedCerts", certificates_new) != IOTHUB_CLIENT_OK)
				{
					printf("failure to set option \"TrustedCerts\"\r\n");
				}
			}
			else {
				if (IoTHubClient_LL_SetOption(iotHubClientHandle, "TrustedCerts", certificates_old) != IOTHUB_CLIENT_OK)
				{
					printf("failure to set option \"TrustedCerts\"\r\n");
				}
			}
...
int main(void)
{
	int judge = 0;//judge the CA version you want
	printf("please input your CA version, 0 for old version and 1 for new version\n");
	scanf("%d", &judge);
    iothub_client_sample_mqtt_run(judge);
    return 0;
}
